...
export function HtmlMinifierTool() {
  ...
  const handleMinify = useCallback(async () => {
    if (!input.trim()) { setError("Please enter some HTML to minify."); setOutput(""); return; }
    setProcessing(true);
    setError("");
    try {
      const res = await fetch("/api/minify", {
        ...
        body: JSON.stringify({ type: "html", code: input }),
      });
    ...
    <div className="space-y-4">
      <div className="grid gap-4 md:grid-cols-2">
        <div className="space-y-2">
          <label className="text-sm font-medium text-foreground">Input HTML</label>
          <textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="Paste your HTML here..." className="h-72 w-full resize-none rounded-lg border border-border bg-muted p-3 font-mono text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary" />
        </div>
        <div className="space-y-2">
