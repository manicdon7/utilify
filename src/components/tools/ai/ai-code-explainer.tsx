...
export function AiCodeExplainer() {
  ...
  const explain = async () => {
    ...
    try {
      const res = await fetch("/api/ai", {
        ...
        body: JSON.stringify({ type: "ai-code-explainer", input: code.trim() }),
      });
  ...
  return (
    <div className="space-y-6">
      <div>
        <label className="mb-2 block text-sm font-medium text-foreground">Paste Your Code</label>
        <textarea
          value={code}
          onChange={(e) => setCode(e.target.value)}
          placeholder="Paste or type code here..."
          rows={12}
          className="w-full rounded-lg border border-border bg-muted font-mono text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary"
        />
      </div>
